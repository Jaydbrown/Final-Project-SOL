...
const Badge = ({ icon: Icon, text }: { icon: any, text: string }) => (
  <div className="flex items-center gap-3 px-6 py-4 bg-white rounded-2xl border border-slate-200 shadow-sm hover:border-emerald-200 transition-colors">
    <div className="p-2 bg-emerald-50 rounded-lg">
      <Icon className="w-5 h-5 sage-text" />
    </div>
    <span className="text-sm font-bold text-slate-700 tracking-tight whitespace-nowrap">{text}</span>
  </div>
);

const TrustBadges: React.FC = () => {
  return (
    <section className="py-12 bg-[#FDFBF7] border-y border-slate-100">
